...
UpdateFactory    = com.hp.hpl.jena.update.UpdateFactory
UpdateExecutionFactory = com.hp.hpl.jena.update.UpdateExecutionFactory
GraphStoreFactory = com.hp.hpl.jena.update.GraphStoreFactory
ValidityReport = com.hp.hpl.jena.reasoner.ValidityReport
PelletOptions = org.mindswap.pellet.PelletOptions
...
module Ontology
  ...
  module KB
	...
	com.hp.hpl.jena.tdb.TDB.sync(@model)
      ...
      def initialize_reasoner
        ...
        validity = @inferred.validate();
        if validity.isValid()
          print "KB eh consistente"
        else
          print "Conflitos encontrados: "
          validity.getReports().each do |i|
            print " - "+ i.description + " " + i.type + "\n"
          end
          explanation = @inferred.getGraph().explainInconsistency();
          print "Explicacao :" + explanation.toString()
        end
      end
  ...
  # Provê uma abstração de update do SPARQL
  class Update
    include KB
    
    # executa a instrução SPARQL sobre o KB
    def exec(command)
      storage.model.begin
      begin
	texto = QUERY_TEMPLATE.sub(/%command%/, command)
	print texto
        instrucao = UpdateFactory.create(texto) 
	store = GraphStoreFactory.create(storage.model)
	qexec = UpdateExecutionFactory.create(instrucao, store);
        qexec.execute
